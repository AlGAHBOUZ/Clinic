...
{
    var patients = await unitOfWork.Repository<Patient>()
        .GetAll()
        .Select(p => new
        {
            p.Id,
    ...
    if (patients.Any())
    {
        ...
        foreach (var patient in patients)
        {
            ...
            var formattedSessions = patientSessions.Select(session => new
            {
                Id = session.Id,
                ServiceName = unitOfWork.Repository<Service>()
...
[HttpPost]
[Route("FilterPatients")]
public IActionResult FilterPatients([FromBody] FilterPatientRequest request)
{
    try
    {
        var parameters = new
        ...
        var patients = unitOfWork.Repository<Patient>().Filter("filter_patient", parameters);
